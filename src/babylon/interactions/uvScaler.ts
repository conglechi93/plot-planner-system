import { PBRMaterial, Texture, Vector3 } from '@babylonjs/core';
...
/** Lấy tất cả Texture thực sự (không null) từ PBRMaterial */
function getPBRTextures(mat: PBRMaterial): Texture[] {
  return [
    mat.albedoTexture,
    mat.bumpTexture,
    mat.metallicTexture,
    ...
    mat.opacityTexture,
    mat.lightmapTexture,
    mat.microSurfaceTexture,
    mat.reflectivityTexture,
...
/**
 * Ghi lại uScale/vScale gốc của tất cả PBR texture trong house.
...
export function snapshotUVForHouse(house: HouseInstance): void {
  ...
  for (const m of house.mesh.getChildMeshes(false)) {
    ...
    const textures = getPBRTextures(m.material);
...
export function recomputeUVForHouse(house: HouseInstance): void {
  ...
  // Tránh chia 0 nếu snapshot scale bị 0 (không xảy ra trong thực tế)
  ...
  for (const m of house.mesh.getChildMeshes(false)) {
    ...
    for (const tex of getPBRTextures(m.material)) {
